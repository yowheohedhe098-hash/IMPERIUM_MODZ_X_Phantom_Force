...
std::string HMG_ENCV2(const std::string& input){
    ...
    return "HMGTEAM" + finalBase64;
}
std::string HMG_DECV2(const std::string& input){
    const std::string prefix = "HMGTEAM";
    if (input.substr(0, prefix.size()) != prefix) {
        throw std::invalid_argument("Invalid encoded string: missing HMGTEAM prefix");
    }
...
bool isBase64(const std::string& text) {
    ...
    std::regex base64Regex("^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$");
